...
async function testInvocation(){
    ...
    describe('Test invocation' , () => {
        ...
        it('should return correct body params' , async() => {
            const postRequestResponse = await axios.post(process.env.dev_WEBHOOKURL_TEST_GET_REQUEST , bodyParams);
        ...
        it('should return that script is paused' , async() => {
            try{
                const Response = await axios.get(process.env.dev_WEBHOOKURL_TEST_GET_REQUEST);
            }catch(err){
